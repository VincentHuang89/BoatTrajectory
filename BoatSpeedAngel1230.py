...
for i in range(0,len(fai)):
    R_ERR=100
    for r in np.arange(1,alpha[i],0.2):
        err=Speed[i]/k_cusp-sin(radians(fai[i]-r))/cos(radians(alpha[i]-r))
        err=abs(err)
        if err<R_ERR:
            r_err[i]=err
            R[i]=r
...
for i in range(0,len(fai)):
    R_ERR=100
    for r in np.arange(1,alpha[i],0.01):
        err=k_cusp-Speed[i]*sin(radians(r))/sin(radians(fai[i]-r))
        err=abs(err)
        if err<R_ERR:
            r_err[i]=err
            R[i]=r
...
res1['R_err']=r_err
res1['R']=R
res1['compare']=res1['R']/res1['alpha']


r_list=list(0.42*res1['alpha'])
ShipSpeed_r=[]
for i in range(0,len(fai)):
    ShipSpeed_r.append(k_cusp*sin(radians(fai[i]-r_list[i]))/sin(radians(r_list[i])))

res1['ShipSpeed_r']=ShipSpeed_r
